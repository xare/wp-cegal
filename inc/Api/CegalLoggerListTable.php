...
use WP_List_Table;
use Inc\cegal\Api\CegaApiDbManager;

class CegalLoggerListTable extends WP_List_Table {

    private $cegalApiDbLoggerManager;

    public function __construct() {
        parent::__construct([
            'singular' => 'cegal_logger',  // Singular label of the table
            'plural'   => 'cegal_loggers', // Plural label of the table
    ...
    public function prepare_items() {
        global $wpdb;
        $cegalApiDbLoggerManager = new CegalApiDbLoggerManager;
        $loggerTable = $wpdb->prefix . $cegalApiDbLoggerManager::CEGAL_LOGGER_TABLE;
        // define data set for WP_List_Table => data
        $where = ''; // Initialize where clause
        if ( isset( $_POST['filter_ean'] ) && !empty( $_POST['filter_ean'] )) {
            $ean = sanitize_text_field( $_POST['filter_ean'] );
            $where = $wpdb->prepare(' WHERE type = %s', $ean );
        }
        if ( isset( $_POST['filter_url'] ) && !empty( $_POST['filter_url'] )) {
            $url = sanitize_text_field( $_POST['filter_url'] );
            $where = $wpdb->prepare(' WHERE action = %s', $url);
        }
        ...
        if ($search_term) {
            ...
            $where = " WHERE ean LIKE '%{$search_term}%'
    ...
    public function wp_list_table_data($where = '', $per_page = 20,  $orderby = '', $order = '', $search_term = '') {
        global $wpdb;
        $geslibApiDbManager = new CegalApiDbManager;
        // Determine what page the user is currently looking at
        $current_page = $this->get_pagenum();
        $offset = ($current_page - 1) * $per_page;

        $loggerTable = $wpdb->prefix . $geslibApiDbManager::CEGAL_LOGGER_TABLE;
